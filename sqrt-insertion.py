...
def insertion_sort(arr):
    for i in range(1, len(arr)):
        key = arr[i]
        j = i - 1
        while j >= 0 and arr[j] > key:  # Ordenar em ordem crescente
            arr[j + 1] = arr[j]
            j -= 1
        arr[j + 1] = key
    return arr
...
def sqrtsort(vetor):
    ...
    while any(partes):  # Enquanto houver elementos nas partes
        maiores = []
        for parte in partes:
            if parte:
                parte_ordenada = insertion_sort(parte)
                maiores.append(parte_ordenada[-1])  # Pega o maior elemento de cada parte
        
        maior_valor = max(maiores)  # Seleciona o maior dentre os maiores elementos
        vetor_solucao.append(maior_valor)
        
        # Remover o maior elemento da parte correspondente
...
def processar_tamanho(tamanho):
    intervalo_maximo = tamanho * 10  # Ajustar o intervalo para garantir a amostra
    vetor_aleatorio = random.sample(range(1, intervalo_maximo), tamanho)

    start_time = time.time()  # Início da medição do tempo
    vetor_ordenado = sqrtsort(vetor_aleatorio)
